...
export function HeroCampusInvitationSection({ onJoin, onSeeCampuses }: HeroCampusInvitationSectionProps) {
  return (
      ...
      <div className="mx-auto max-w-6xl overflow-hidden rounded-[2rem] bg-foreground px-6 py-9 text-background sm:px-10 sm:py-12">
        <div className="grid items-center gap-8 md:grid-cols-[1fr_auto]">
          ...
          <div className="flex flex-col items-start gap-3 md:items-end">
            ...
            <Button
              onClick={onSeeCampuses}
              variant="ghost"
              className="group rounded-full px-7 text-background hover:bg-background/10 hover:text-background"
            >
              See the campus roadmap
              <ArrowRight className="h-4 w-4 transition-transform group-hover:translate-x-1" aria-hidden="true" />
            </Button>
          </div>
        </div>
